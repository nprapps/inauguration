...
(function($){$.livechatwidget=function(element,options){var defaults={scribble_host:'apiv1.scribblelive.com',chat_id:null,chat_token:null,update_interval:1000};var plugin=this;plugin.settings={};plugin.$root=$(element);plugin.init=function(){plugin.$root.html(JST.widget());plugin.settings=$.extend({},defaults,options||{});plugin.update_live_chat();setInterval(plugin.update_live_chat,plugin.settings.update_interval);};plugin.update_live_chat=function(){$.ajax({url:'http://'+plugin.settings.scribble_host+'/event/'+plugin.settings.chat_id+'/page/0?format=json&Token='+plugin.settings.chat_token+'&Type=TEXT',dataType:'jsonp',cache:false,success:function(response){var posts=[];_.each(response.Posts,function(post){if(post.Type==='TEXT')posts.push(post);});plugin.render_post(posts);}});};plugin.render_post=function(posts){var post=posts[0];var template=JST.widget_text({post:post});plugin.$root.find('p').html(template);};plugin.init();};$.fn.livechatwidget=function(options){return this.each(function(){if($(this).data('livechatwidget')===undefined){var plugin=new $.livechatwidget(this,options);$(this).data('livechatwidget',plugin);}});};}(jQuery));
$(function(){$('#live-chat-widget').livechatwidget({chat_id:'74796',chat_token:'FtP7wRfX',update_interval:10000});});